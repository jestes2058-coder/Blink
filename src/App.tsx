...
const CompatibilityMatrix = lazy(
  () => import("./components/CompatibilityMatrix"),
)
...
export default function App() {
  ...
  const handleIncomingSOS = useCallback(
    (req: BloodRequest) => {
      const currentUser = store.getCurrentUser()
      if (!currentUser) return
      if (
        req.requestorId === currentUser.id ||
        (currentUser.phone && req.requestorPhone === currentUser.phone)
      ) {
        return
      }
      ...
      if (
        ...
        addToast(
          ...
          req.urgency === "critical"
            ? "🚨 EMERGENCY BLOOD ALERT IN YOUR AREA!"
            : `🩸 Blood Request in ${req.district}`,
      ...
      if (isSupabaseConfigured) {
        ...
        if (newRequests.length > prevRequests.length) {
          const fresh = newRequests.filter(
            (nr) => !prevRequests.some((pr) => pr.id === nr.id),
          )
      ...
      deferredPrompt.userChoice.then((choiceResult: any) => {
        if (choiceResult.outcome === "accepted") {
          addToast(
            "success",
            "App Installed",
            "B-Link has been added to your home screen!",
          )
        }
  ...
  const userBloodGroup = (user.bloodGroup ||
    ...
    "O+") as BloodGroup
  const userDistrict = (user.district || myProfile?.district || "")
    .trim()
    .toLowerCase()
  ...
  const pendingCount = matchingRequests.filter((r) => {
    const match = r.matches.find(
      (m) => m.donorId === user.id || (myProfile && m.donorId === myProfile.id),
    )
    return !match || match.status === "pending"
  }).length

  // Find most urgent active Critical SOS for top banner alert
  const activeCriticalSOS = matchingRequests.find(
    ...
    <div className="min-h-screen flex flex-col bg-[#FFF8F8] text-[#1A0505] relative">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="font-semibold text-white">
                  <strong>{activeCriticalSOS.bloodGroup}</strong> needed for{" "}
                  {activeCriticalSOS.patientName} at{" "}
                  {activeCriticalSOS.hospital} ({activeCriticalSOS.district})
                </span>
              </div>
            </div>
      ...
      >
        <Suspense fallback={<LoadingFallback />}>
          ...
          {view === "request-blood" && (
            <RequestBlood user={user} setView={setView} onToast={addToast} />
          )}

          {view === "my-requests" && (
            <MyRequests user={user} setView={setView} onToast={addToast} />
          )}

          {view === "notifications" && (
            <Notifications user={user} setView={setView} onToast={addToast} />
          )}
          ...
          {view === "donors-directory" && (
            <DonorsDirectory user={user} setView={setView} />
          )}
      ...
      <nav
        ...
        className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-red-100 px-3 py-2 flex items-center justify-around shadow-lg"
      >
        ...
        {showSOSModal && (
          <EmergencySOSModal
            ...
            onSuccess={() => {
              addToast(
                "warning",
                "🚨 Urgent SOS Broadcasted",
                "All eligible donors in district have been alerted with emergency priority!",
              )
              setView("my-requests")
            }}
          />
          ...
          <IncomingRequestAlertModal
            ...
            onAccept={(req) => {
              ...
              addToast(
                "info",
                "Opening Request",
                `Viewing details for ${req.patientName}.`,
              )
            }}
