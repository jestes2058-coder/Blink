...
export default function MyRequests({ user, setView, onToast }: Props) {
  const [filterStatus, setFilterStatus] =
    useState<"all" | "open" | "fulfilled">("all")
  ...
  const myRequests = requests
    .filter((r) => {
      ...
      if (
        ...
        !user.name.match(/^\d+$/) &&
        r.requestorName &&
        r.requestorName.toLowerCase() === user.name.toLowerCase()
      )
        return true
      return false
    })
  ...
  function handleMarkFulfilled(req: BloodRequest) {
    ...
    onToast(
      "success",
      "Request Fulfilled",
      `Marked request for ${req.patientName} as fulfilled.`,
    )
    refresh()
  }
  ...
  function handleDelete(id: string) {
    if (
      confirm("Are you sure you want to cancel and remove this blood request?")
    ) {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-red-100 text-xs sm:text-sm">
              Track live donor notifications, responses, and revealed contact
              numbers.
            </p>
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-3xl border border-gray-100 p-10 sm:p-12 text-center shadow-sm max-w-md mx-auto space-y-3">
          ...
          <p className="text-xs text-gray-600">
            When you create a blood requirement, it will appear here with live
            match statuses.
          </p>
          <button
            onClick={() => setView("request-blood")}
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <UserAvatar
                      src={req.requestorAvatar || user.avatar}
                    ...
                    <div className="h-2.5 bg-gray-200 rounded-full overflow-hidden flex">
                      {accepted.length > 0 && (
                        <div
                          ...
                          style={{
                            width: `${(accepted.length / total) * 100}%`,
                          }}
                        />
                      )}
                      {pending.length > 0 && (
                        <div
                          ...
                          style={{
                            width: `${(pending.length / total) * 100}%`,
                          }}
                        />
                      )}
                      {declined.length > 0 && (
                        <div
                          ...
                          style={{
                            width: `${(declined.length / total) * 100}%`,
                          }}
                        />
                      )}
                    </div>
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {req.matches.map((match) => {
                        ...
                        const donorObj = store
                          .getDonors()
                          .find((d) => d.id === match.donorId)
