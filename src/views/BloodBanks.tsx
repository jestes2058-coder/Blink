...
export default function BloodBanks({ setView }: Props) {
  ...
  const filteredBanks = bloodBanks.filter((bank) => {
    const matchState = searchState === 'ALL' || !bank.state || bank.state === searchState
    const matchDistrict = searchDistrict === 'ALL' || bank.district === searchDistrict
    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 overflow-x-hidden">
      ...
      <div className="bg-gradient-to-r from-rose-600 to-red-700 text-white rounded-3xl p-5 sm:p-6 mb-6 shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold">24/7 National Emergency Blood Helpline</h2>
            <p className="text-xs text-red-100">Direct hospital dispatch & emergency ambulance coordination</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-red-100 shadow-sm p-4 sm:p-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3 flex-wrap flex-1">
          <div className="flex-1 min-w-[160px]">
            <label htmlFor="searchStateBank" className="block text-[10px] font-bold uppercase tracking-wider text-gray-700 mb-1">State</label>
            <select
              id="searchStateBank"
              name="state"
              value={searchState}
          ...
          <div className="flex-1 min-w-[160px]">
            <label htmlFor="searchDistrictBank" className="block text-[10px] font-bold uppercase tracking-wider text-gray-700 mb-1">District</label>
            <select
          ...
          >
            <div>
              <div className="flex items-start justify-between gap-2 mb-3">
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-gray-900 leading-snug">{bank.name}</h2>
              ...
              <div className="flex items-center gap-4 text-xs text-gray-600 mb-4 pb-3 border-b border-gray-100">
                ...
                <span className="font-semibold text-red-700">{bank.district}{bank.state ? `, ${bank.state}` : ''}</span>
              </div>
            ...
            <div className="pt-5 border-t border-gray-100 flex items-center gap-3 mt-4">
              ...
              <button
                onClick={() => alert(`Directions to ${bank.name} (${bank.address}) will open in Google Maps.`)}
