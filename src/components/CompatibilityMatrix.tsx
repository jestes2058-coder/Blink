...
import { Droplet, ArrowRight, ArrowLeft, Info, Sparkles, CheckCircle2, XCircle } from 'lucide-react'
import type { BloodGroup } from '../types'
import { BLOOD_GROUPS, COMPATIBLE_DONORS, COMPATIBLE_RECIPIENTS } from '../store'
    ...
    <div className="bg-white rounded-3xl border border-red-100 shadow-sm p-5 sm:p-7">
      <div className="flex items-start justify-between mb-5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-lg sm:text-xl font-bold text-gray-900" style={{ fontFamily: "'DM Serif Display', serif" }}>
              Blood Group Compatibility Explorer
            </h2>
          </div>
          <p className="text-xs sm:text-sm text-gray-500">
            Select any blood group to instantly check medical transfusion compatibility.
          </p>
        </div>
      </div>
      ...
      {isUniversalDonor && (
        <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-red-600 to-rose-700 text-white shadow-md flex items-center gap-3">
          ...
          <div className="text-xs sm:text-sm">
            <span className="font-bold">Universal Red Cell Donor:</span> O- blood can be safely transfused to patients of ANY blood group! Crucial for emergency trauma surgeries.
          </div>
        </div>
      )}
      ...
      {isUniversalRecipient && (
        <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-amber-600 to-orange-700 text-white shadow-md flex items-center gap-3">
          <Sparkles className="w-6 h-6 text-yellow-300 flex-shrink-0" />
          <div className="text-xs sm:text-sm">
            <span className="font-bold">Universal Recipient:</span> AB+ individuals can safely receive red blood cells from ANY blood group donor!
          </div>
        </div>
      )}
      ...
      {activeTab === 'donate' ? (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-gray-400">
              {recipients.length === 8 ? 'Universal Donor' : `${recipients.join(', ')}`}
            </span>
          </div>
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm ${
                      isCompatible ? 'bg-emerald-200 text-emerald-900' : 'bg-gray-200 text-gray-600'
                    }`}>
                      {target}
                    </span>
                  ...
                  {isCompatible ? (
                    <span className="text-xs font-bold text-emerald-700 flex items-center gap-1">
                      <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Match
                    </span>
                  ) : (
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-gray-400">
              {donors.length === 8 ? 'Universal Recipient' : `${donors.join(', ')}`}
            </span>
          </div>
      ...
      <div className="mt-5 p-3.5 rounded-2xl bg-gray-50 border border-gray-100 flex items-start gap-2.5 text-xs text-gray-600">
        ...
        <p>
          <span className="font-semibold text-gray-800">Hospital Protocol Note:</span> In urgent medical situations, exact group matching is prioritized, but universal compatibility rules enable emergency cross-group transfusions when local supply is scarce.
        </p>
      </div>
    </div>
  )
